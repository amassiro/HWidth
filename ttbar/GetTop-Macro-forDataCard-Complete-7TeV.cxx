...
// #include "test/HWidth/ttbar/plotUtils.C"
...
int GetTop_Macro_forDataCard_Complete_7TeV(int iWP, std::string suffix = "of", std::string whatAnalysis = "cut", bool doEnrichToGetEvents = false, int binCutPt = 0, int binTCHE = 0) {
...
//  int _debug = 4;
 ...
 std::string inputDirectory = "output_Latinos_tree_2011_2orMoreJet2011_Top";
 std::string fileSamples = "test/Latinos11/ttbar/datacard/SamplesH.txt";
 
//  std::string inputDirectory = "output_Latinos_tree_2012_2orMoreJet2012_MVA_added"; // "output_Latinos_tree_2012_2orMoreJet2012_Top";
//  std::string fileSamples = "test/HWidth/ttbar/datacard/SamplesH.txt";
 ...
 v_commonCut_WW.push_back("pt2>10"); // >10
...
//  v_commonCut_WW.push_back("(bveto_ip==1 && nbjettche==0)");
 ...
 ///---- 2011 specific ----
 v_commonCut_WW.push_back("abs(jeteta1)<4.5 && abs(jeteta2)<4.5");
 ///---- 2011 specific ----
 
 //---- 2 or 3 jets 
 v_commonCut_WW.push_back("(njet >= 2 && njet <= 3 && njetvbf == 0) ");
 
 v_commonCut_WW.push_back("abs(eta1 - (jeteta1+jeteta2)/2)/detajj < 0.5 && abs(eta2 - (jeteta1+jeteta2)/2)/detajj < 0.5");
...
//   v_commonCut_Higgs.push_back("BDTG_weights_testVariables_MVAHWidth > 0.40");  //---- 0.55   0.30  0.40  -0.10 = crazy test!
//  }
 
 //---- VBF cuts 
 if (whatAnalysis == "shape-7TeV") {
  v_commonCut_Higgs.push_back("mjj>500");
  v_commonCut_Higgs.push_back("detajj>2.5");
 }  

 //---- the following are taken care by CHITOP
//  ptll>45
...
//  etaBins.push_back (2.0); //--> before disabled
 ...
 TString nameFileInEff = Form ("test/HWidth/ttbar/result-eff-%s-%s-pt-%d-tche-%d.txt",whatAnalysis.c_str(),suffix.c_str(),binCutPt,binTCHE);
 ...
 else
 {
  ...
  TString nameFileOutEff = Form ("test/HWidth/ttbar/result-eff-%s-%s-pt-%d-tche-%d.txt",whatAnalysis.c_str(),suffix.c_str(),binCutPt,binTCHE);
...
//      (err_DATA_Sub.at(iZone)).at(iBin) = (err_DATA.at(iZone)).at(iBin);
 ...
 for (int iBin=0; iBin<nBin; iBin++) {
  ...
  TString nameFileOutEffHiggs = Form ("test/HWidth/ttbar/result-eff-%s-%s-pt-%d-tche-%d-%d.txt",whatAnalysis.c_str(),suffix.c_str(),binCutPt,binTCHE,iWP);
...
//   std::cout << "; err_DD_Top = " << (err_DD_Top.at(0)).at(iBin) << std::endl;
 ...
 TString nameFileOut = Form ("test/HWidth/ttbar/result-%s-%s-pt-%d-tche-%d-%d.txt",whatAnalysis.c_str(),suffix.c_str(),binCutPt,binTCHE,iWP);
...
//  myfileAll                   << mH[iWP]                << "   " ;
 ...
 nameFileOut = Form ("test/HWidth/ttbar/result-SF-%s-%s-pt-%d-tche-%d-%d.txt",whatAnalysis.c_str(),suffix.c_str(),binCutPt,binTCHE,iWP);
 std::cout << " cat " << nameFileOut.Data() << std::endl;
 std::cout << std::endl;
